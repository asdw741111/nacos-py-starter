...
def default_timeout_fallback_func(ex, *args,**kwargs):
    # raise ex
    return ("function time out " + str(ex) +
        "  function info: " + str(args)+ " " + str(kwargs))
...
class FuncFuse(object):
    ...
    """
    def __init__(self, fallback_func=default_fall_back_func ,
    ...
    def __go_func(self, tid, fn, fuse_status):
        try:
            ...
            fc = self.except_fallback_func(ex, fn.__name__,
                *self.__args,**self.__kwargs)
            self._func_time[fn.__name__ + str(tid)] = fc
            ec = self._func_fuse[fn]["error"] + 1
            if fuse_status:self._func_fuse[fn]["error"] = ec

    def __is_fuse(self,fn,fuse_status, except_percent,
        time_windows, time_count):
    ...
    def fuse(self, timeout=6, fuse_status=False, except_percent=0.5,
        time_windows=5, time_count=2 ):
        def get_f(fn):
            def get_args(*args,**kwargs):
                self.__kwargs = kwargs
                self.__args   = args
                # 判断是否需要熔断，默认不熔断
                if self.__is_fuse(fn, fuse_status,except_percent,
                    time_windows, time_count):
                    return self.fallback_func(fn.__name__, *args, **kwargs)
                tid = time.time()
                func_thread = threading.Thread(target=self.__go_func, args=(tid,
                    fn, fuse_status))
                ...
                try:
                    ...
                    tff = self.timeout_fallback_func(TimeoutError,
